...
class Actor(object):
    ...
    def build_model(self):
        ...
        #What is partial
        default_dense = partial(tf.layers.dense, kernel_regularizer = kernel_regularizer)

        #What is variabl_scope
        with tf.variable_scope(self.name) as scope:
            ...
            hid3 = default_dense(hid2, 128)
            hid4 = default_dense(hid3, 256)
            action = default_dense(hid4, self.n_action, activation=tf.nn.tanh, use_bias=False)
...
class Critic(object):
    ...
    def build_model(self):
        activation = tf.nn.relu
        kernel_regularizer = tf.contrib.layers.l2_regularizer(0.1)
        default_dense = partial(tf.layers.dense, activation=activation, kernel_regularizer=kernel_regularizer)

        with tf.variable_scope(self.name) as scope:
            observation = tf.placeholder(tf.float32, shape=[None, self.n_observation])
            action = tf.placeholder(tf.float32, shape=[None, self.n_action])
            hid1 = default_dense(observation,32)
            hid1_1 = default_dense(hid1, 64)

            hid2 = default_dense(action, 32)
            hid2_1 = default_dense(hid2, 64)

            hid3 = tf.concat([hid1_1, hid2_1], axis=1)
            hid4 = default_dense(hid3, 256)
    ...
    def predict_Q(self, obs_batch, action_batch):
        return self.Q.eval(session=self.sess, feed_dict={self.observation:obs_batch, self.action:action_batch})

    def compute_action_grads(self, obs_batch, action_batch):
        return self.action_grads.eval(session=self.sess, feed_dict={self.observation:obs_batch, self.action:action_batch})

    def train(self, obs_batch, action_batch, Qexpected_batch):
        self.train_op.run(session=self.sess, feed_dict={self.observation:obs_batch, self.action:action_batch, self.Qexpected:Qexpected_batch})
...
class AsyncNets(object):
    ...
    def __build_async_assign(self):
        ...
        keys = net_dict.keys()
        async_update_op = [target_net_dict[key].assign((1-self.TAU)*target_net_dict[key]+self.TAU*net_dict[key]) for key in keys]
...
with tf.Session() as sess:
    ...
    while episode < max_episode:
        ...
        if episode < max_explor_eps:
            p = episode / max_explor_eps
        ...
        if iteration >= memory_warmup:
            ...
            Qexpected_batch = reward_batch + gamma*(1-done_batch)*Q_next
